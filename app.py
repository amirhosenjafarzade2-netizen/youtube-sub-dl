...
def validate_url(url):
    ...
    try:
        ...
        if is_handle or is_ch:
            ...
            # Append /videos to scope to uploads tab only (excludes shorts/community posts)
...
def get_info(url, is_playlist=False, cookies_file=None):
    """Get video IDs and titles. Handles playlists, channels, and single videos."""
    if is_playlist or is_channel_url(url):
        ...
        with YoutubeDL(ydl_opts) as ydl:
            ...
            # Channels can have a nested entries list (tabs → videos)
            ...
            titles = [entry.get('title', f'video_{i+1}') for i, entry in enumerate(entries) if entry and entry.get('id')]
            title = result.get('title', 'channel_subtitles')
...
def main():
    ...
    st.markdown("Download subtitles from YouTube videos, playlists, and channels! Supports Turkish/EN.")
        ...
        **For Age-Restricted Videos**: Upload cookies to bypass blocks.
        ...
        if url:
            try:
                ...
                if url_type == 'channel':
                    st.info("📺 Channel URL detected — will download subtitles for all videos.")

                if url_type == 'both':
                    download_scope = st.selectbox("Scope", ["Entire Playlist", "Single Video"], key="scope")

                is_playlist_mode = url_type in ['playlist', 'channel'] or (url_type == 'both' and download_scope == "Entire Playlist")
                if is_playlist_mode:
        ...
        with st.spinner("Fetching..."):
            try:
                ...
                if url_type == 'both' and download_scope == 'Entire Playlist':
                    ...
                    selected_url = playlist_url  # holds channel URL for 'channel' type
                    is_playlist = True
                else:
                    selected_url = video_url
                    is_playlist = False

                type_label = {'playlist': 'Playlist', 'channel': 'Channel', 'video': 'Video', 'both': 'Playlist'}.get(url_type, 'URL')
                ...
                if not subtitle_files:
                    st.error("No subs found. Try a different URL or upload cookies for restricted content.")
                    return
